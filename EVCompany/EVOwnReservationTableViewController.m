//
//  EVOwnReservationTableViewController.m
//  EVCompany
//
...
@interface EVOwnReservationTableViewController (){
    UILabel *labelTitle;
...
- (void)UserReservation{
    ...
    [dict setValue:currentStartTime forKey:@"startDate"];
    [dict setValue:currentEndTime forKey:@"expiryDate"];
    NSLog(@"Retrieving Own Reservation Parameter Data : %@",dict);
    
    [[EVOCPPClientApi sharedOcppClient] getReservationListWithParameters:dict Success:^(id result) {
        [self.delegate hideModal];
        NSLog(@"Retrieving Own Reservation Response Data : %@",result);
        if ([result[@"status"] isEqualToString:@"true"]) {
            _reservation = [result[@"reservationList"] mutableCopy];
            [self.tableView reloadData];
        }else
        {
            [_reservation removeAllObjects];
            [self.delegate hideModal];
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:@"You don't have any reservations on this charger." delegate:self cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
            [alert setTag:6];
            [alert show];
            [self.tableView reloadData];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.row)
    {
        ...
        case 5:
            ...
            // labelDetails.text = [[_reservation objectAtIndex:indexPath.section] objectForKey:@"reservation_Status"];
            [cell.contentView addSubview:labelTitle];
            [cell.contentView addSubview:labelDetails];
            break;
        ...
        case 6:
        {
            if([[[_reservation objectAtIndex:indexPath.section] objectForKey:@"reservation_Status"] isEqualToString:@"InProgress"]){
                ...
                if(![[[_reservation objectAtIndex:indexPath.section] objectForKey:@"transactionStatus"] isEqualToString:@"0"] ){
                    ...
                    //   [stop_button setBackgroundColor:[UIColor colorWithRed:42.0/255.0 green:175.0/255.0 blue:61.0/255.0 alpha:1]];
...
- (void)cancelButtonClick:(id) sender{
    [self.delegate showModalWithMessage:@"Cancelling your reservation..."];
    ...
    NSLog(@"Cancel Reservation Parameter Data : %@",dict);
    
    [[EVOCPPClientApi sharedOcppClient] getCancelReservationWithParameters:dict Success:^(id result) {
        NSLog(@"Cancel Reservation Responce Data : %@",result);
        if ([result[@"status"] isEqualToString:@"true"]) {
            [self.delegate showModalWithMessage:@"Reservation cancelled!" forDuration:1.5];
            [_reservation removeObjectAtIndex:senderBtn.tag];
            //[self.tableView reloadData];
            [self fetchConnectorUsingChargerId];
            [self refreshAction];
            [self.tableView reloadData];
           // UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Message" message:@"Reservation canceled successfully." delegate:self cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
           // [alert show];
        }else
        {
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    if ([alertView tag] == 0) {
        if (buttonIndex == 0) {
            [self.delegate showModalWithMessage:@"Starting..." forDuration:1.5];
